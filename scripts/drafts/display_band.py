...
def normalize_array(data_16bit, p2, p98, gamma=0.6):
    """
    Applies a dynamic range stretch and gamma correction using pre-computed bounds.
    """
...
for color in ["blue", "green", "red"]:
    ...
    with rasterio.open(f"aletsch_{color}.tif") as src:
        bands_data[color] = normalize_array(src.read(1), p2_global, p98_global, gamma=0.6)
...
rgb_composite = np.dstack((bands_data["red"], bands_data["green"], bands_data["blue"]))
...
# No cmap argument is passed; imshow automatically interprets (H, W, 3) arrays as RGB
ax_blue.imshow(blue_isolated, extent=extent)
ax_blue.set_title("Blue Band (B02)")

ax_green.imshow(green_isolated, extent=extent)
ax_green.set_title("Green Band (B03)")

ax_red.imshow(red_isolated, extent=extent)
ax_red.set_title("Red Band (B04)")

# 7. Render RGB Composite (Row 2)
ax_rgb.imshow(rgb_composite, extent=extent)
...
output_pdf = "aletsch_multispectral_isolated.pdf"
plt.tight_layout()
